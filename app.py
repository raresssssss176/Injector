...
def search_and_pay():
    ...
    try:
        ...
        # Using double {{ }} to ensure literal brackets reach Stripe
        success_url = (
...
def success():
    ...
    if not session_id or "{CHECKOUT_SESSION_ID}" in session_id:
        return "Error: Stripe session not initialized correctly.", 400
    ...
    if filtered_ads:
        try:
            ...
            # Using a simple connect/send to keep it fast
            mail.send(msg)
        except Exception as e:
            # We print the error to the Render log so we can see it, but we don't crash the page
